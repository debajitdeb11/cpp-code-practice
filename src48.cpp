...
of car or applying brakes will stop the car but he does not know about how on pressing accelerator the speed is actually increasing, 
...
Class helps us to group data members and member functions using available access specifiers. A Class can decide which data member 
...
For example, consider the pow() method present in math.h header file. Whenever we need to calculate power of a number,
we simply call the function pow() present in the math.h header file and pass the numbers as arguments without knowing 
...
class ImplementAbstraction{
    ...
    public:
        ...
        int display(){       // int display() can be replaced with void display() then the return type is not required!
            cout << "a = " << a << endl;
            cout << "b = " << b << endl;
